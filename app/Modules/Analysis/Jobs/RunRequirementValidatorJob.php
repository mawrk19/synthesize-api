...
class RunRequirementValidatorJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(AnalysisService $analysisService): void
    {
        ...
        $result = $analysisService->runValidator($document);
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        Log::error('Requirement validator failed', ['run_id' => $this->runId, 'message' => $exception?->getMessage()]);
        $run->update([
            'status' => DocumentStatus::Failed,
            'error_message' => $exception?->getMessage() ?? 'Validation failed.',
        ]);
    }
}
